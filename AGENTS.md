...
**Git:** stay on `dev` for all work — commit/push there only. Never create or switch branches. Promote `dev` → `main` only when the user asks to deploy. If not on `dev`, ask — do not switch yourself.
