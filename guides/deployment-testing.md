...
- `readiness` — runs `scripts/repo-readiness.sh` against the Postgres 16 service. It checks whitespace, formatting, compilation, Credo, tests, a production release build, and the disposable MCP tool smoke. Compiler warnings are reported but do not block the gate; formatting, Credo, tests, release build, and smoke failures do.
