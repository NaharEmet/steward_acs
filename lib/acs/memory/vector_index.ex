defmodule Acs.Memory.VectorIndex do
  ...
  """
  def upsert_embedding(memory_id, embedding, repo \\ Acs.Repo) when is_binary(memory_id) and is_list(embedding) do
  ...
  """
  @spec search_similar([float()], keyword(), module()) :: [%{memory_id: String.t(), similarity: float()}]
  def search_similar(embedding, options \\ [], repo \\ Acs.Repo) when is_list(embedding) and is_list(options) do
      ...
      {:ok, %{rows: rows}} when is_list(rows) ->
        ...
        |> Enum.map(fn [memory_id, embedding_json] ->
          case Jason.decode(embedding_json) do
            {:ok, emb} -> %{memory_id: memory_id, similarity: Acs.Memory.Embedding.cosine_similarity(embedding, emb)}
            _ -> nil
          end
        end)
  ...
  """
  @spec search_threshold([float()], float(), module()) :: [%{memory_id: String.t(), similarity: float()}]
  def search_threshold(embedding, threshold, repo \\ Acs.Repo) when is_list(embedding) and is_number(threshold) do
    embedding
