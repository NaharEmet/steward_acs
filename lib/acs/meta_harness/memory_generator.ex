defmodule Acs.MetaHarness.MemoryGenerator do
  ...
  def generate_memories(opts \\ []) do
    ...
    failing =
      Enum.filter(analysis.tool_reliability, fn {_, d} ->
        d.success_rate < 1 - @error_threshold
      end)

    findings =
      findings ++
        Enum.map(failing, fn {name, data} ->
          {:high_failure_rate, name, data}
        end)

    # Slow tools
    slow =
      Enum.filter(analysis.latency_analysis, fn {_, d} ->
        (d.p95_latency || 0) > @latency_threshold_ms
      end)

    findings =
      findings ++
        Enum.map(slow, fn {name, data} ->
          {:slow_tool, name, data}
        end)
  ...
  defp save_memory({:high_failure_rate, tool_name, data}) do
    summary =
      "High failure rate: #{tool_name} (#{(data.success_rate * 100.0) |> Float.round(1)}%)"
