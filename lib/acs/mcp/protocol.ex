defmodule Acs.MCP.Protocol do
  ...
  def handle_message(message, agent_role \\ nil, agent_org_id \\ nil, agent_permissions \\ nil)

  def handle_message(message, agent_role, agent_org_id, agent_permissions) when is_binary(message) do
    case Jason.decode(message) do
      {:ok, decoded} -> handle_message(decoded, agent_role, agent_org_id, agent_permissions)
  ...
  def handle_message(
    ...
    params = msg["params"] || %{}
    handle_request(id, method, params, agent_role || "admin", agent_org_id, agent_permissions)
  end

  def handle_message(%{"jsonrpc" => "2.0", "method" => method} = msg, _agent_role, _agent_org_id, _agent_permissions) do
    params = msg["params"] || %{}
    handle_notification(method, params)
  end

  def handle_message(%{"jsonrpc" => "2.0"} = _msg, _agent_role, _agent_org_id, _agent_permissions) do
    {:ok, error_response(nil, -32600, "Invalid Request", "Missing method")}
  end

  def handle_message(_msg, _agent_role, _agent_org_id, _agent_permissions) do
  ...
  defp handle_request(id, "initialize", _params, _agent_role, _agent_org_id, _agent_permissions) do
    result = %{
  ...
  defp handle_request(id, "tools/list", _params, agent_role, _agent_org_id, _agent_permissions) do
    tools = ToolRegistry.list_tools_mcp(agent_role)
    {:ok, success_response(id, %{"tools" => tools})}
  end

  defp handle_request(id, "tools/call", params, agent_role, agent_org_id, agent_permissions) do
    name = params["name"]
  ...
  defp handle_request(id, "ping", _params, _agent_role, _agent_org_id, _agent_permissions) do
    {:ok, success_response(id, %{})}
  end

  # OpenCode startup methods — return empty/success responses to avoid "Method not found" errors
  defp handle_request(id, "config.providers", _params, _agent_role, _agent_org_id, _agent_permissions) do
    {:ok, success_response(id, %{"providers" => []})}
  end

  defp handle_request(id, "provider.list", _params, _agent_role, _agent_org_id, _agent_permissions) do
    {:ok, success_response(id, %{"providers" => []})}
  end

  defp handle_request(id, "app.agents", _params, _agent_role, _agent_org_id, _agent_permissions) do
    {:ok, success_response(id, %{"agents" => []})}
  end

  defp handle_request(id, "config.get", _params, _agent_role, _agent_org_id, _agent_permissions) do
    {:ok, success_response(id, %{})}
  end

  defp handle_request(id, method, _params, _agent_role, _agent_org_id, _agent_permissions) do
