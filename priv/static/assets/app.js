...
`
  ...
  document.addEventListener("click", (event) => {
    const closeButton = event.target instanceof Element ? event.target.closest("[data-toast-close]") : null;
    if (closeButton) dismissToast(closeButton.closest("[data-toast]"));
  });
  document.addEventListener("click", async (event) => {
    const trigger = event.target instanceof Element ? event.target.closest("[data-copy-target], [data-copy-value]") : null;
    if (!trigger) return;
